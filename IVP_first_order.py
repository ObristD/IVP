...
def euler(h, n, x0, y, reference, x, but, r_fun, stps, prnt):
    error = []
    ref_error = []
    y_ret = []
    for i in range(1, n + 1):
        k1 = dydx(x0 + but[0][0]*h, y)

        y = y + h*(but[1][1]*k1)  # Update y
        x0 = x0 + h  # Update x

        error.append(abs(y - reference))
        ref_error.append(reference_function(y, x0))
        y_ret.append(y)

        print("y({})\t= {}   Error: {} | step:".format(format(x0, ".2e"),
                                                       "\033[1m" + colored(format(y, ".2e"), "magenta") +
                                                       "\033[0;0m", colored(format(error[-1], ".2e"), "red")), i) if stps else None

        print("Error to reference fun:\t", colored(format(ref_error[-1], ".2e"), "red"), "| step:", i,
              "\n") if r_fun else None

    if not stps:
        print("y({})\t= {} \t Error: {}".format(round(x0, 3),
                                                "\033[1m" + colored(format(y, ".2e"), "magenta") + "\033[0;0m",
...
def rk2(h, n, x0, y, reference, x, but, r_fun, stps, prnt):
    error = []
    ref_error = []
    y_ret = []
    for i in range(1, n + 1):
        k1 = dydx(x0 + but[0][0]*h, y)
        k2 = dydx(x0 + but[1][0]*h, y + but[1][1]*h * k1)

        y = y + h*(but[2][1]*k1 + but[2][2]*k2)  # Update y
...
def rk3(h, n, x0, y, reference, x, but, r_fun, stps, prnt):
    error = []
    ref_error = []
    y_ret = []
    for i in range(1, n + 1):
        k1 = dydx(x0 + but[0][0] * h, y)
        k2 = dydx(x0 + but[1][0] * h, y + but[1][1] * k1 * h)
        k3 = dydx(x0 + but[2][0] * h, y + but[2][1] * k1 * h + but[2][2] * k2 * h)

        y = y + h * (k1 * but[3][1] + k2 * but[3][2] + k3 * but[3][3])  # Update y
        x0 = x0 + h  # Update x
...
def rk4(h, n, x0, y, reference, x, but, r_fun, stps, prnt):
    error = []
    ref_error = []
    y_ret = []
    for i in range(1, n + 1):
        ...
        k2 = dydx(x0 + but[1][0]*h, y + but[1][1]*k1*h)
        k3 = dydx(x0 + but[2][0]*h, y + but[2][1]*k1*h + but[2][2]*k2*h)
        k4 = dydx(x0 + but[3][0]*h, y + but[3][1]*k1*h + but[3][2]*k2*h + but[3][3]*k3*h)

        y = y + h*(k1*but[4][1] + k2*but[4][2] + k3*but[4][3] + k4*but[4][4])  # Update y
        x0 = x0 + h  # Update x
...
def method(x0, y0, x, n, h, reference, meth, but, r_fun, stps, prnt=True):
    err_list = []
    ref_error_list = []
    y_retruns = []
    if h is not None:
        ...
        if meth == "euler":
            ...
            for e in range(len(n)):
                ...
                y = y0
                residual = euler(h[e], n[e], x0, y, reference, x, but, r_fun, stps, prnt)
        ...
        if meth == "rk2" or meth == "heun" or meth == "ralston":
            ...
            for e in range(len(n)):
                ...
                print("Step width:", colored(format(float(h[e]), ".2e"), "blue") + "\n---------") if prnt else None
                y = y0
                residual = rk2(h[e], n[e], x0, y, reference, x, but, r_fun, stps, prnt)
                err_list.append(residual[0])
                ref_error_list.append(residual[1])
                y_retruns.append(residual[2])
        ...
        if meth == "rk3":
            ...
            for e in range(len(n)):
                ...
                print("Step width:", colored(format(float(h[e]), ".2e"), "blue") + "\n---------") if prnt else None
                y = y0
                residual = rk3(h[e], n[e], x0, y, reference, x, but, r_fun, stps, prnt)
                err_list.append(residual[0])
                ref_error_list.append(residual[1])
                y_retruns.append(residual[2])
        ...
        if meth == "rk4":
            ...
            for e in range(len(n)):
                ...
                print("Step width:", colored(format(float(h[e]), ".2e"), "blue") + "\n---------") if prnt else None
                y = y0
                residual = rk4(h[e], n[e], x0, y, reference, x, but, r_fun, stps, prnt)
                err_list.append(residual[0])
                ref_error_list.append(residual[1])
                y_retruns.append(residual[2])
...
def plot_residuals(sol, logaxis, invertx, inverty, w_err, met):
    stepwidth = sol[0]
    errors = [sol[w_err][i][-1] for i in range(len(sol[1]))]
    gca = plt.gca()
    if logaxis:
...
def n_for_error_smaller_than(r_err, w_err, x0_, y_, x_, reference_, metho, butc, uplmt):
    ...
    for i1 in range(1, uplmt, 10000):
        res1 = method(x0_, y_, x_, [i1], h=None, reference=reference_,
                      meth=metho, but=butc, r_fun=False, stps=False, prnt=False)
...
def difference_of_solutions(sol, first, second, absolute=True):
    ind1 = ""
    ind2 = ""
    if first != -1:
        ind1 = first + 1
...
def reference_function(y, x):
    y_x = (17/2*exp(3*x**2) - 0.5)**(1/3)
    return abs(y - y_x)  # Reference function | use numpy (np.cos())


def dydx(x, y):
    return sqrt(y)  # Differential function
...
One of either n_ or h_ must be None, otherwise only h_ will be taken in to account
Methods:
euler | rk2 | rk3 | rk4 | heun | ralston 
"""
x0_ = 0                     # Initial value of x
y_ = 4                      # Initial value of y -> y(x0)
x_ = 10                      # X final
n_ = [10, 100, 200, 300, 10000, 100000]     # Number of iterations | Type: List / NoneType
h_ = None                 # Step with  | Type: List / NoneType
reference_ = (17/2*exp(3*2**2) - 0.5)**(1/3)             # reference value
method_ = "euler"           # Method
show_steps = False          # Show Steps | Type Bool
ref_fun = False             # Steps with reference Function! | Type Bool


butch = get_butch(method_)
solution = method(x0_, y_, x_, n_, h_, reference_, method_, butch, ref_fun, show_steps)
...
"""
# difference_of_solutions(solution, first=-1, second=-2, absolute=True)
...
"""
reference_error = 1e-3  # eg. 1e-3 = 10^-3 = 0.001
...
"""
w_e = 1  # 1: Error to value | 2: Error to function
